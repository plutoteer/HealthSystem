...
router.afterEach((to, from) => {
  ...
  // 排除不需要重置滚动位置的页面（如登录页、注册页等）
  const excludePaths = ['/login', '/register', '/change_Password']
  if (excludePaths.includes(to.path)) {
    return
  }

  // 重置 .app-main 的滚动位置，确保每次路由切换后页面从顶部开始显示
  // 使用多次延迟确保在组件完全渲染后执行
  const resetScroll = () => {
...
function myFilterAsyncRoutes(menuList) {
  return menuList.filter(menu => { // 对menuList数组进行过滤操作
    try {
      ...
      // 组件加载失败时，记录错误并过滤掉该菜单项，避免导致整个路由系统崩溃
      console.error(`加载组件失败: ${menu.component}`, error)
      return false // 过滤掉加载失败的组件
    }
  })
}
